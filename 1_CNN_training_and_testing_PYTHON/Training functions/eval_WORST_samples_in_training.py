        ...
        if len(batch_x) == batch_size:
           ...
           if epochs % plot_every == 0:
              ...
              if bool_plot_sens:
                   ...
                   if sensitivity < 0.8:
                       # m,M = input_im.min(),input_im.max()
                       # input_im = ((input_im - m) / (M - m))
                       # input_im = np.asarray(input_im[:, :, :, 0] * 255, np.uint8)
                       # input_max = np.amax(input_im, axis=0)
                       # imsave(s_path +  str(int(i)) +'_input_max.tif', input_max)
                       # imsave(s_path +  str(int(i)) +'_input_im.tif', input_im)
                      
                       # imsave(s_path +  str(int(i)) +'_truth_im.tif', np.asarray(truth_im[:, :, :, 1] * 255, np.uint8))   
                       
                       # imsave(s_path +  str(int(i)) +'_truth_im_max.tif', np.asarray(np.amax(truth_im[:, :, :, 1], axis=0) * 255, np.uint8))  
                   
                       all_idx_low_sens.append(i)
                   ...
                   if sensitivity < 0.8:            
                       # m,M = input_im_val.min(),input_im_val.max()
                       # input_im_val = ((input_im_val - m) / (M - m))
                       # input_im_val = np.asarray(input_im_val[:, :, :, 0] * 255, np.uint8)
                       # input_max_val = np.amax(input_im_val, axis=0)
                       
                       # imsave(s_path + str(int(i)) +'_input_max_val.tif', input_max_val)
                       
                       # imsave(s_path + str(int(i)) +'_input_im_val.tif', input_im_val)
                       # #truth_im_val = np.asarray(truth_im_val, np.uint8)
                       
                       # imsave(s_path + str(int(i)) +'_truth_im_val.tif',  np.asarray(truth_im_val[:, :, :, 1] * 255, np.uint8))      
                       
                       # imsave(s_path +  str(int(i)) +'_truth_im_val_max.tif',  np.asarray(np.amax(truth_im_val[:, :, :, 1], axis=0) * 255, np.uint8))  
...
np.save('all_idx_low_sens_0_8', all_idx_low_sens)             
