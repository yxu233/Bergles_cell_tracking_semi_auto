            ...
            for x in range(1, width + quad_size, round(quad_size - quad_size * overlap_percent)):
                 ...
                 for y in range(1, height + quad_size, round(quad_size - quad_size * overlap_percent)):
                      ...
                      for z in range(1, depth_im + quad_depth, round(quad_depth - quad_depth * overlap_percent)):
                          ...
                          """ do bcolz """
                          if empty:                               
                               a = bcolz.carray(quad_intensity, expectedlen = 5000, rootdir = sav_dir + 'input_im')
                               print(a.chunklen)
                               b = bcolz.carray(quad_truth, expectedlen = 5000,  rootdir = sav_dir + 'truth_im')
                               empty = 0
                          else:
                               a.append(quad_intensity)
                               a.flush()

                               b.append(quad_intensity)
                               b.flush()
...
input_batch = X[0:num_iter]
truth_batch = Y[0:num_iter]
