...
for fileNum = 1:2:numfids
    ...
    for loop_idx = 1:length(idx_original_seed)
        ...
        length(all_indices)
        depth_limit = 10000;
        [all_original_seeds, save_linear_indices] = recurse_branch_tree_paranodes(originalIdx, all_original_seeds, sortedS, depth_limit, size(input_im), save_linear_indices);
    ...
    %% Plot paranodes as well
    
    paranodes = all_original_seeds > 1;
    paranodes = imdilate(paranodes, strel('sphere', 2));
    
    paranodes = imresize(paranodes,scale_factor, 'method', 'nearest');
    mip = max(paranodes, [], 3); mip = im2double(mip);
    imwrite(mip, strcat(filename_save, '_seed_end_', int2str(fileNum), '_paranodes_MAX.tif') , 'writemode', 'append', 'Compression','none')
    ...
    for k = 1:length(all_original_seeds_size_limited(1,1, :))
        ...
        input = paranodes(:, :, k);
        imwrite(input, strcat(filename_save,'_seed_end_', int2str(fileNum), '_paranodes.tif') , 'writemode', 'append', 'Compression','none')
